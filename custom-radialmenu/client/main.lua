...
AddEventHandler('qb-radialmenu:lockpickVehicle', function()
    local coords = GetEntityCoords(cache.ped)
    local vehicle = wsb.getClosestVehicle(vec3(coords.x, coords.y, coords.z), 5.0, false)
    if not vehicle or not DoesEntityExist(vehicle) then
        TriggerEvent('qb:notify', Strings.vehicle_not_found, Strings.vehicle_not_found_desc, 'error')
    else
        ...
        if dist < 2.5 then
            ...
            TriggerEvent('qb:notify', Strings.too_far, Strings.too_far_desc, 'error')
        end
    end
end)
